...
def geocode(address):
    ...
    geocoding = {'key': key,
                 ...
                 'keywords': address}
    res = requests.get(
        "https://restapi.amap.com/v3/place/text?parameters", params=geocoding)
    if res.status_code == 200:
        json = res.json()
        status = json.get('status')
        result = json.get('pois')
        if status == '1' and len(result) >= 1:
            geocodes = json.get('pois')[0]
            province = str(geocodes.get('pname')) #省份
            city = str(geocodes.get('cityname'))#城市
            district = str(geocodes.get('adname'))#区
            name = str(geocodes.get('name')) #街道
            lng = geocodes.get('location').split(',')[0]
            lat = geocodes.get('location').split(',')[1]
            return [province , city , district , name , lng , lat]
        else:
            print('状态：'+str(json.get('info')))
            return None
    else:
        return None
    ...
    if len(csv_list) >=1:
        ...
        for address in csv_list:
            ...
            if results == None:
                ...
                lng,lat = gcj02_to_wgs84(float(results[4]), float(results[5]))#火星转GPS
                txtfile.write(str(lng)+'\t'+str(lat)+'\t')
            txtfile.write('\n')
            csvFile.close()
            print(N,results)
            N += 1
